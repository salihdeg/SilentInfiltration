...
namespace Climbing
{
    ...
    public class MovementCharacterController : MonoBehaviour
    {
        ...
        [Header("OuterStepFunc")]
        [SerializeField] private bool _customStep = false;
        ...
        private void StepClimb()
        {
            ...
            if (Physics.Raycast(_stepRayLower.transform.position, transform.TransformDirection(Vector3.forward), out hitLower, 0.1f))
            {
                ...
                {
                    rb.position -= new Vector3(0f, -_stepSmooth, 0f);
                }
            }
            ...
            if (Physics.Raycast(_stepRayLower.transform.position, transform.TransformDirection(-1.5f, 8, 1), out hitLowerMinus45, 0.1f))
            {
                RaycastHit hitUpperMinus45;
                if (Physics.Raycast(_stepRayUpper.transform.position, transform.TransformDirection(-1.5f, 8, 1), out hitUpperMinus45, 0.2f))
                {
                    rb.position = new Vector3(0f, -_stepSmooth, 0f);
                }
            }
        ...
        public bool CheckBoundaries()
        {
            ...
            float right = 0.25f;
            float lenght = 1.5f; //default 1
            //Throws raycasts down to detect limits of surface
            if (!controller.characterDetection.ThrowRayOnDirection(origin, Vector3.down, lenght))
                ret = CheckSurfaceBoundary();
            else if (!controller.characterDetection.ThrowRayOnDirection(origin + transform.right * right, Vector3.down, lenght) && ret == false)
                ret = CheckSurfaceBoundary();
            else if (!controller.characterDetection.ThrowRayOnDirection(origin + transform.right * -right, Vector3.down, lenght) && ret == false)
                ret = CheckSurfaceBoundary();
            ...
            if (showDebug)
            {
                ...
                Debug.DrawLine(origin, origin + Vector3.down * lenght);
                Debug.DrawLine(origin + transform.right * right, origin + transform.right * right + Vector3.down * lenght);
                Debug.DrawLine(origin + transform.right * -right, origin + transform.right * -right + Vector3.down * lenght);
            }
